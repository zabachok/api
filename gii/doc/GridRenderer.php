...
class GridRenderer
{
    ...
    public function render(): string
    {
        ...
        $result = '| ' . str_pad('Поле', $columnSizes['attribute'], ' ') . '  |  ' . str_pad('Тип',
                $columnSizes['rule'], ' ') .
            '   | Обязательно | Описание | ' . str_pad('По умолчанию', $columnSizes['default'], ' ') . ' |' . PHP_EOL;

        $result .= '|-' . str_pad('-', $columnSizes['attribute'], '-') . '-|-' . str_pad('-', $columnSizes['rule'],
                '-') .
            '-|-------------|----------|-' . str_pad('-', $columnSizes['default'], ' ') . '-|' . PHP_EOL;
        ...
        foreach ($this->rules as $attribute => $rule) {
            ...
            $result .= ' | - | ';
            $result .= isset($this->default[$attribute]) ? $this->renderDefault($this->default[$attribute]) : ' - ';
            $result .= ' |' . PHP_EOL;
        }
     ...
     * @param array $rule
     */
    private function defaultFiller(array $attributes, array $rule): void
    {
        foreach ($attributes as $attribute) {
            $this->default[$attribute][] = $rule['value'];
        }
    }
     ...
     * @param array $rule
     */
    private function booleanFiller(array $attributes, array $rule): void
    {
        foreach ($attributes as $attribute) {
            $this->rules[$attribute][] = 'boolean';
        }
    }
     ...
     * @return string
     */
    private function renderDefault(array $values): string
    {
        $result = [];
