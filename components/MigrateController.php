...
class MigrateController extends YiiMigrateController
{
    ...
    protected function getNewMigrations()
    {
        ...
        foreach ($this->getMigrationPaths() as $namespace => $migrationPath) {
            ...
            while (($file = readdir($handle)) !== false) {
                ...
                $directory = str_replace($this->migrationPath[0], '', $migrationPath);
    ...
    protected function createMigration($class): Migration
    {
        ...
        $file = $this->migrationPath[0] . DIRECTORY_SEPARATOR . $path . '.php';
        require_once($file);
    ...
    {
        return $this->migrationPath[0] . date('/Y/m');
    }
    ...
    {
        $files = [];
        $files[] = $this->migrationPath[0];
        $files = array_merge($files, FileHelper::findDirectories($this->migrationPath[0], ['recursive' => true]));
